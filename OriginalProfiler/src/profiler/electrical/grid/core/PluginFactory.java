...
public class PluginFactory {
	...
	public boolean create(Document document, Node profilerNode) {
		...
		if (profilerNode.getAttributes().getNamedItem("release") != null)
			pluginRelease = profilerNode.getAttributes()
					.getNamedItem("release").getNodeValue();
		else
		...
		Class<?> pluginHandlerClass = getHandlerClass("profiler.electrical.grid.plugins."
				+ plugin + "." + plugin);
		...
		try {
			...
			for (int i = 0; i < execute.length; i++) {
				if (execute[i].getName().equals("execute")) {
					execute[i].invoke(pluginHandlerClass.newInstance(),
							parametersValues);
					break;
				}
			}
