...
public class OvenFullHandler extends DeviceHandler {

	OvenFull oven;
	int mission;
	boolean notify;
	
	public OvenFullHandler (ActionMaker actionMaker, OvenFull oven, int mission, boolean notify){
	...
	public void startDevice(RecipeLine recipeLine) {
		...
		if (recipeLine.getStartTime() == 0){
			if (specialHandling[0].equals("OFF"))
				oven.mode = OvenFull.Mode.OFF;
			else if (specialHandling[0].equals("ON"))
				oven.mode = OvenFull.Mode.ON;
		}
		else{
