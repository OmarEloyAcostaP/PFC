...
public class CookingStoveFullHandler extends DeviceHandler {

	CookingStoveFull cookingStove;
	int mission;
	boolean notify;
	
	public CookingStoveFullHandler (ActionMaker actionMaker, CookingStoveFull cookingStove, int mission, boolean notify){
	...
	public void startDevice(RecipeLine recipeLine) {
		...
		if (recipeLine.getStartTime() == 0){
			if (specialHandling[0].equals("OFF"))
				cookingStove.mode = CookingStoveFull.Mode.OFF;
			else if (specialHandling[0].equals("ON"))
				cookingStove.mode = CookingStoveFull.Mode.ON;
		}
		else{
