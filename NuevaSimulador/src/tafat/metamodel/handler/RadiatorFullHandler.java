...
public class RadiatorFullHandler extends DeviceHandler {

	RadiatorFull radiator;
	int mission;
	boolean notify;
	
	public RadiatorFullHandler (ActionMaker actionMaker, RadiatorFull radiator, int mission, boolean notify){
	...
	public void startDevice(RecipeLine recipeLine) {
		...
		if (recipeLine.getStartTime() == 0){
			if (specialHandling[0].equals("OFF"))
				radiator.mode = RadiatorFull.Mode.OFF;
			else if (specialHandling[0].equals("ON"))
				radiator.mode = RadiatorFull.Mode.ON;
		}
		else{
